...
// MULTIPLICAR
let botonMult=document.getElementById("multi")
//Cuando se le da click al botonSumar etonces el ejecuta la siguiente funcion
botonMult.addEventListener("click", function () {
    ...
    resultado=parseInt(numeroUno)*parseInt(numeroDos)
    let impresionResultado = document.getElementById("resultadoCalculadora")
    impresionResultado.textContent= `El resultado de la Multiplicaion es: ${resultado}`
})
// DIVIDIR
...
botonDivision.addEventListener("click", function () {
    ...
    if (num2 !=0){
        resultado=parseInt(numeroUno)/parseInt(numeroDos)
        let impresionResultado = document.getElementById("resultadoCalculadora")
    impresionResultado.textContent= `El resultado de la Division es es: ${resultado}`
    }
    else{
        if(num2==0){
        let impresionResultado = document.getElementById("resultadoCalculadora")
        impresionResultado.textContent= `No es posible Dividir entre 0`}
        else{
            impresionResultado.textContent= `No es posible Dividir entre 0`}  
        }
    }
